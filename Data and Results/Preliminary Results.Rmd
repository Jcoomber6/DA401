...
## Early Results
* ADD IMPLICATIONS
* Results should be from models from the methods section
* report al important parts of results - not just P values
* inlcudes numbers titles, captions and cite in text (Table 1)!!!!
* link prior research to this section - cite things
...
TelehealthandPopulation_VehicleInternetHospital <- TelehealthandPopulation_VehicleInternetHospital %>%
  mutate(
   HospitalCountPer100k = (hospital_count / POP) * 100000  # Normalize per 100,000 people
  )
...
TelehealthandPopulation_VehicleInternet %>%
  ...
  geom_bar(stat = "identity") +
  geom_text(aes(label = scales::comma(TelehealthVisitsPer100k)), 
            vjust = -0.5, size = 5) +  # Add text labels above bars
...
TelehealthandPopulation_VehicleInternetHospital %>%
  arrange(desc(HospitalCountPer100k)) %>%  # Arrange in descending order
  ggplot(aes(x = fct_reorder(State, HospitalCountPer100k, .desc = TRUE), y = HospitalCountPer100k, fill = State)) +
  geom_bar(stat = "identity") +
...
TelehealthandPopulation_VehicleInternetHospital %>%
    ...
    ggplot(aes(x = RuralUrban, y = TelehealthVisitsPer100k, fill = RuralUrban)) +
    geom_bar(stat = "identity") +
    labs(title = "Telehealth Visits (Per 100k people in each state) by Rural vs. Urban", 
         x = "Category of State", 
...
```{r}
model1 <- lm(TelehealthVisitsPer100k ~ TotalOwnedPer100k + TotalRentPer100k + WithInternetPer100k +
               HospitalCountPer100k + GDP + UNRATE, 
             TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # without State/Year fixed effects

model2 <- lm(TelehealthVisitsPer100k ~ TotalOwnedPer100k + TotalRentPer100k + WithInternetPer100k +
               HospitalCountPer100k + GDP + UNRATE + State, 
             TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State fixed effects

model3 <- lm(TelehealthVisitsPer100k ~ TotalOwnedPer100k + TotalRentPer100k + WithInternetPer100k +
               HospitalCountPer100k + GDP + UNRATE + State + Year, 
...
coefz <- c("TotalOwnedPer100k" = "Vehicles Owned",
           "TotalRentPer100k" = "Renters",
           "WithInternetPer100k" = "Internet Access",
           "HospitalCountPer100k" = "Hospitals",
           "GDP" = "GDP",
...
## Total Vehicles Owned Per 100k

```{r}

model1<-lm(TelehealthVisitsPer100k~TotalOwnedPer100k, TelehealthandPopulation_VehicleInternetHospital) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~TotalOwnedPer100k + State, TelehealthandPopulation_VehicleInternetHospital) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~TotalOwnedPer100k + State + Year, TelehealthandPopulation_VehicleInternetHospital) # with State and Year fixed effects
...
modelsummary(regz,
             title = "Effect of State and Year on the Number of Cars Owned (Per 100k people) impact on Number of Telehelath Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz)
...
* No Fixed Effect R^2 = 0.002 suggests the number of telehealth visits is explained little by how many vehicles owned.

* State Fixed Effect R^2 = 0.126 suggests adding state fixed effects improves the model’s explanatory power.

* State and Year Fixed Effect R^2 = 0.135 suggests including both state and year fixed effects improves the model slightly even more, explaining more variation in number of telehealth visits due to number of vehicles owned.

* When State and Year fixed effects are included, the relationship between number of vehicles owned and telehealth visits changes from -0.014*** to -0.094***. This suggests that once controlling for these factors, more cars owned still has a negative relationship with the number of telehealth visitssuggesting higher number of vehicles owned is associated with fewer telehealth visits. This could be because individuals with more vehicles have better access to in-person healthcare and don't need to use telehealth.


## Total Vehicles Rent Per 100k

```{r}
model1<-lm(TelehealthVisitsPer100k~TotalRentPer100k, TelehealthandPopulation_VehicleInternetHospital) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~TotalRentPer100k + State, TelehealthandPopulation_VehicleInternetHospital) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~TotalRentPer100k + State + Year, TelehealthandPopulation_VehicleInternetHospital) # with State and Year fixed effects

regz <- list(`No Fixed Effect` = model1,
             `State Fixed Effect` = model2,
             `State and Year Fixed Effect` = model3)

coefz <- c("TotalRentPer100k" = "Total Rent")
...
modelsummary(regz,
             title = "Effect of State and Year on the Number of Cars Rented (Per 100k people) Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz) 
...
* No Fixed Effect R^2 = 0.000 suggests the number of telehealth visits is unexplained by the number of rented vehicles.

* State Fixed Effect R^2 = -0.079 suggests adding state fixed effects improves the model’s explanatory power

* State and Year Fixed Effect R^2 = 0.134 suggests including both state and year fixed effects improves the model slightly even more, explaining more variation in number of telehealth visits due to number of vehicles rented.

* When State and Year fixed effects are included, the relationship between number of vehicles owned and telehealth visits changes from 0.000 to 0.060***. This suggests that once controlling for these factors, more cars rented has a positive relationship with the number of telehealth visits suggesting higher number of vehicles rented is associated with more telehealth visits. This could be because individuals renting vehicles have lower accessibility to in person visits and would use telehealth

* This result is positive, different from the negative relationship shown with owning vehicles and telehalth visits, suggesting that those who rent cars use telehealth more than those who own cars.

## With Internet

```{r}
model1<-lm(TelehealthVisitsPer100k~WithInternetPer100k, TelehealthandPopulation_VehicleInternetHospital) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~WithInternetPer100k + State, TelehealthandPopulation_VehicleInternetHospital) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~WithInternetPer100k + State + Year, TelehealthandPopulation_VehicleInternetHospital) # with State and Year fixed effects

regz <- list(`No Fixed Effect` = model1,
             `State Fixed Effect` = model2,
             `State and Year Fixed Effect` = model3)

coefz <- c("WithInternetPer100k" = "With Internet")
...
modelsummary(regz,
             title = "Effect of State and Year on the Number of People with Internet Access (Per 100k people) Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz) 
...
* No Fixed Effect R^2 = 0.013 suggests the number of telehealth visits is explained little by having Internet Access

* State Fixed Effect R^2 = 0.132 suggests adding state fixed effects improves the model’s explanatory power

* State and Year Fixed Effect R^2 = 0.136 suggests including both state and year fixed effects improves the model slightly more, explaining more variation in number of telehealth visits due to having internet.

* When State and Year fixed effects are included, the relationship between having internet and telehealth visits changes from -0.036*** to -0.056***. This suggests that once controlling for these factors, having internet still has a negative relationship with the number of telehealth visits, and it appears more negative once controlling for state and year.


## Hospital Count

```{r}
model1<-lm(TelehealthVisitsPer100k~HospitalCountPer100k, TelehealthandPopulation_VehicleInternetHospital) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~HospitalCountPer100k + State, TelehealthandPopulation_VehicleInternetHospital) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~HospitalCountPer100k + State + Year, TelehealthandPopulation_VehicleInternetHospital) # with State and Year fixed effects

regz <- list(`No Fixed Effect` = model1,
             `State Fixed Effect` = model2,
             `State and Year Fixed Effect` = model3)

coefz <- c("HospitalCountPer100k" = "Hospital Count")
...
modelsummary(regz,
             title = "Effect of State and Year on The Number of Hospitals in each state (Per 100k people) Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz) 
...
* No Fixed Effect R^2 = 0.004 suggests the number of telehealth visits is explained little by the number of hospitals per 100k people in that state.

* State Fixed Effect R^2 = 0.090 suggests adding state fixed effects improves the model’s explanatory power

* State and Year Fixed Effect R^2 = 0.133 suggests including both state and year fixed effects improves the model more, explaining more variation in number of telehealth visits due to the number of hospitals per 100k people in that state.

* When State and Year fixed effects are included, the relationship between the number of hospitals and telehealth visits changes from -34.734*** to -423.203***. This suggests that once controlling for these factors, the number of hospitals still has a negative relationship with the number of telehealth visits, and it appears more much more negative once controlling for state and year. 

* This suggests that more hospitlas has a lower use of telehelath visits. This could be beacause of more availability of hospitals to treat patients without telehealth.


## GDP Fixed Effect

```{r}
model1<-lm(TelehealthVisitsPer100k~GDP, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~GDP + State, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~GDP + State + Year, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State and Year fixed effects

regz <- list(`No Fixed Effect` = model1,
             `State Fixed Effect` = model2,
             `State and Year Fixed Effect` = model3)

coefz <- c("GDP" = "Gross Domestic Product (GDP)")
...
modelsummary(regz,
             title = "Effect of State and Year on GDP's Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz) 
...
```{r}
model1<-lm(TelehealthVisitsPer100k~UNRATE, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # without Income fixed effects
model2 <- lm(TelehealthVisitsPer100k~UNRATE + State, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State fixed effects
model3<-lm(TelehealthVisitsPer100k~UNRATE + State + Year, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State and Year fixed effects

regz <- list(`No Fixed Effect` = model1,
             `State Fixed Effect` = model2,
             `State and Year Fixed Effect` = model3)

coefz <- c("UNRATE" = "Unemployement Rate")
...
modelsummary(regz,
             title = "Effect of State and Year on Unemployement Rate's Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_map = coefz) 
