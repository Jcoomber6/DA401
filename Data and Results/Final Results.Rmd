...
# Load the data
Telehealth <- read.csv("Telehealth Dataset/Medicare Telehealth Trends.csv", header = TRUE)
...
```{r}
# Load the data
Population <- read.csv("Population Dataset/Annual Estimates of the Resident Population for the United States.csv", header = TRUE, sep = ",")
...
```{r}
Vehicle2023 <- read.csv("Vehicle Datasets/Tenure by Vehicles Available_2023.csv", header = TRUE, sep = ",")
```
...
```{r}
Vehicle2022 <- read.csv("Vehicle Datasets/Tenure by Vehicles Available_2022.csv", header = TRUE, sep = ",")
```
...
```{r}
Vehicle2021 <- read.csv("Vehicle Datasets/Tenure by Vehicles Available_2021.csv", header = TRUE, sep = ",")
```
...
```{r}
Vehicle2020 <- read.csv("Vehicle Datasets/Tenure by Vehicles Available_2020.csv", header = TRUE, sep = ",")
```
...
```{r}
InternetSubs2023 <- read.csv("Internet Datasets/Presence and Types of Internet Subscriptions_2023.csv", header = TRUE, sep = ",")
```
...
```{r}
InternetSubs2022 <- read.csv("Internet Datasets/Presence and Types of Internet Subscriptions_2022.csv", header = TRUE, sep = ",")
```
...
```{r}
InternetSubs2021 <- read.csv("Internet Datasets/Presence and Types of Internet Subscriptions_2021.csv", header = TRUE, sep = ",")
```
...
```{r}
InternetSubs2020 <- read.csv("Internet Datasets/Presence and Types of Internet Subscriptions_2020.csv", header = TRUE, sep = ",")
```
...
```{r}
Hospitals <- read.csv("Hospital Dataset/CentersForMedicareAndMedicaidServicesFacilities Hospitals.csv", header = TRUE, sep = ",")
```
...
```{r}
# Load the data
GDP <- read.csv("Economics Datasets/GDP.csv", header = TRUE, sep = ",")
UNRATE <- read.csv("Economics Datasets/UNRATE.csv", header = TRUE, sep = ",")
...
```{r}
Transit <- read.csv("Transportation Dataset/Transit Report Card.csv", header = TRUE, sep = ",")
...
## Cleaning Poverty Level Datasets

### Poverty 2023 

```{r}
Poverty2023 <- read.csv("Poverty Datasets/Poverty Status in the past 12 months_2023.csv", header = TRUE, sep = ",")
```


```{r}
# Transpose using pivot_longer and pivot_wider
Poverty_transposed2023 <- Poverty2023 %>%
  t() %>%
  as.data.frame(stringsAsFactors = FALSE)

# Store original column names as a new column
Poverty_transposed2023 <- Poverty_transposed2023 %>%
  mutate(Original_Column_Names = rownames(Poverty_transposed2023))

# Set first row as column names
colnames(Poverty_transposed2023) <- Poverty_transposed2023[1, ]

# Remove the first row, keeping the new column names
Poverty_transposed2023 <- Poverty_transposed2023[-1, ] 

# Remove all non-letter characters from column names
colnames(Poverty_transposed2023) <- gsub("[^A-Za-z]", "", colnames(Poverty_transposed2023))

colnames(Poverty_transposed2023) <- make.unique(colnames(Poverty_transposed2023))

Poverty_transposed2023 <- Poverty_transposed2023 %>% rename(State = LabelGrouping)

# Remove "Margin.of.Error"
Poverty_transposed2023 <- Poverty_transposed2023[!grepl("Margin.of.Error", rownames(Poverty_transposed2023)), ]

# Remove "..Total..Estimate"
Poverty_transposed2023 <- Poverty_transposed2023[!grepl("..Total..Estimate", rownames(Poverty_transposed2023)), ]

# Remove "..Below.poverty.level..Estimate"
Poverty_transposed2023 <- Poverty_transposed2023[!grepl("..Below.poverty.level..Estimate", rownames(Poverty_transposed2023)), ]

# Rename row names by removing "..Percent.below.poverty.level..Estimate"
rownames(Poverty_transposed2023) <- gsub("\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate", "", rownames(Poverty_transposed2023))

Poverty_transposed2023 <- Poverty_transposed2023 %>%
  mutate(State = str_replace(State, "\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate$", ""))

Poverty_transposed2023 <- Poverty_transposed2023[, c("State", "Populationforwhompovertystatusisdetermined")]

```


```{r}
Poverty_transposed2023$State <- gsub("District.of.Columbia", "District of Columbia", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("Puerto.Rico", "Puerto Rico", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("West.Virginia", "West Virginia", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("New.Hampshire", "New Hampshire", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("North.Carolina", "North Carolina", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("South.Carolina", "South Carolina", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("Rhode.Island", "Rhode Island", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("North.Dakota", "North Dakota", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("South.Dakota", "South Dakota", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("New.Mexico", "New Mexico", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("New.Jersey", "New Jersey", Poverty_transposed2023$State)
Poverty_transposed2023$State <- gsub("New.York", "New York", Poverty_transposed2023$State)
```

### Poverty 2022

```{r}
Poverty2022 <- read.csv("Poverty Datasets/Poverty Status in the past 12 months_2022.csv", header = TRUE, sep = ",")
```

```{r}
# Transpose using pivot_longer and pivot_wider
Poverty_transposed2022 <- Poverty2022 %>%
  t() %>%
  as.data.frame(stringsAsFactors = FALSE)

# Store original column names as a new column
Poverty_transposed2022 <- Poverty_transposed2022 %>%
  mutate(Original_Column_Names = rownames(Poverty_transposed2022))

# Set first row as column names
colnames(Poverty_transposed2022) <- Poverty_transposed2022[1, ]

# Remove the first row, keeping the new column names
Poverty_transposed2022 <- Poverty_transposed2022[-1, ] 

# Remove all non-letter characters from column names
colnames(Poverty_transposed2022) <- gsub("[^A-Za-z]", "", colnames(Poverty_transposed2022))

colnames(Poverty_transposed2022) <- make.unique(colnames(Poverty_transposed2022))

Poverty_transposed2022 <- Poverty_transposed2022 %>% rename(State = LabelGrouping)

# Remove "Margin.of.Error"
Poverty_transposed2022 <- Poverty_transposed2022[!grepl("Margin.of.Error", rownames(Poverty_transposed2022)), ]

# Remove "..Total..Estimate"
Poverty_transposed2022 <- Poverty_transposed2022[!grepl("..Total..Estimate", rownames(Poverty_transposed2022)), ]

# Remove "..Below.poverty.level..Estimate"
Poverty_transposed2022 <- Poverty_transposed2022[!grepl("..Below.poverty.level..Estimate", rownames(Poverty_transposed2022)), ]

# Rename row names by removing "..Percent.below.poverty.level..Estimate"
rownames(Poverty_transposed2022) <- gsub("\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate", "", rownames(Poverty_transposed2022))

#Correct State values
Poverty_transposed2022 <- Poverty_transposed2022 %>%
  mutate(State = str_replace(State, "\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate$", ""))

Poverty_transposed2022 <- Poverty_transposed2022[, c("State", "Populationforwhompovertystatusisdetermined")]

```


```{r}
Poverty_transposed2022$State <- gsub("District.of.Columbia", "District of Columbia", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("Puerto.Rico", "Puerto Rico", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("West.Virginia", "West Virginia", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("New.Hampshire", "New Hampshire", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("North.Carolina", "North Carolina", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("South.Carolina", "South Carolina", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("Rhode.Island", "Rhode Island", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("North.Dakota", "North Dakota", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("South.Dakota", "South Dakota", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("New.Mexico", "New Mexico", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("New.Jersey", "New Jersey", Poverty_transposed2022$State)
Poverty_transposed2022$State <- gsub("New.York", "New York", Poverty_transposed2022$State)
```

### Poverty 2021

```{r}
Poverty2021 <- read.csv("Poverty Datasets/Poverty Status in the past 12 months_2021.csv", header = TRUE, sep = ",")
```

```{r}
# Transpose using pivot_longer and pivot_wider
Poverty_transposed2021 <- Poverty2021 %>%
  t() %>%
  as.data.frame(stringsAsFactors = FALSE)

# Store original column names as a new column
Poverty_transposed2021 <- Poverty_transposed2021 %>%
  mutate(Original_Column_Names = rownames(Poverty_transposed2021))

# Set first row as column names
colnames(Poverty_transposed2021) <- Poverty_transposed2021[1, ]

# Remove the first row, keeping the new column names
Poverty_transposed2021 <- Poverty_transposed2021[-1, ] 

# Remove all non-letter characters from column names
colnames(Poverty_transposed2021) <- gsub("[^A-Za-z]", "", colnames(Poverty_transposed2021))

colnames(Poverty_transposed2021) <- make.unique(colnames(Poverty_transposed2021))

Poverty_transposed2021 <- Poverty_transposed2021 %>% rename(State = LabelGrouping)

# Remove "Margin.of.Error"
Poverty_transposed2021 <- Poverty_transposed2021[!grepl("Margin.of.Error", rownames(Poverty_transposed2021)), ]

# Remove "..Total..Estimate"
Poverty_transposed2021 <- Poverty_transposed2021[!grepl("..Total..Estimate", rownames(Poverty_transposed2021)), ]

# Remove "..Below.poverty.level..Estimate"
Poverty_transposed2021 <- Poverty_transposed2021[!grepl("..Below.poverty.level..Estimate", rownames(Poverty_transposed2021)), ]

# Rename row names by removing "..Percent.below.poverty.level..Estimate"
rownames(Poverty_transposed2021) <- gsub("\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate", "", rownames(Poverty_transposed2021))

#Correct State values
Poverty_transposed2021 <- Poverty_transposed2021 %>%
  mutate(State = str_replace(State, "\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate$", ""))

Poverty_transposed2021 <- Poverty_transposed2021[, c("State", "Populationforwhompovertystatusisdetermined")]

```


```{r}
Poverty_transposed2021$State <- gsub("District.of.Columbia", "District of Columbia", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("Puerto.Rico", "Puerto Rico", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("West.Virginia", "West Virginia", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("New.Hampshire", "New Hampshire", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("North.Carolina", "North Carolina", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("South.Carolina", "South Carolina", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("Rhode.Island", "Rhode Island", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("North.Dakota", "North Dakota", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("South.Dakota", "South Dakota", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("New.Mexico", "New Mexico", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("New.Jersey", "New Jersey", Poverty_transposed2021$State)
Poverty_transposed2021$State <- gsub("New.York", "New York", Poverty_transposed2021$State)
```

### Poverty 2020

```{r}
Poverty2020 <- read.csv("Poverty Datasets/Poverty Status in the past 12 months_2020.csv", header = TRUE, sep = ",")
```

```{r}
# Transpose using pivot_longer and pivot_wider
Poverty_transposed2020 <- Poverty2020 %>%
  t() %>%
  as.data.frame(stringsAsFactors = FALSE)

# Store original column names as a new column
Poverty_transposed2020 <- Poverty_transposed2020 %>%
  mutate(Original_Column_Names = rownames(Poverty_transposed2020))

# Set first row as column names
colnames(Poverty_transposed2020) <- Poverty_transposed2020[1, ]

# Remove the first row, keeping the new column names
Poverty_transposed2020 <- Poverty_transposed2020[-1, ] 

# Remove all non-letter characters from column names
colnames(Poverty_transposed2020) <- gsub("[^A-Za-z]", "", colnames(Poverty_transposed2020))

colnames(Poverty_transposed2020) <- make.unique(colnames(Poverty_transposed2020))

Poverty_transposed2020 <- Poverty_transposed2020 %>% rename(State = LabelGrouping)

# Remove "Margin.of.Error"
Poverty_transposed2020 <- Poverty_transposed2020[!grepl("Margin.of.Error", rownames(Poverty_transposed2020)), ]

# Remove "..Total..Estimate"
Poverty_transposed2020 <- Poverty_transposed2020[!grepl("..Total..Estimate", rownames(Poverty_transposed2020)), ]

# Remove "..Below.poverty.level..Estimate"
Poverty_transposed2020 <- Poverty_transposed2020[!grepl("..Below.poverty.level..Estimate", rownames(Poverty_transposed2020)), ]

# Rename row names by removing "..Percent.below.poverty.level..Estimate"
rownames(Poverty_transposed2020) <- gsub("\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate", "", rownames(Poverty_transposed2020))

#Correct State values
Poverty_transposed2020 <- Poverty_transposed2020 %>%
  mutate(State = str_replace(State, "\\.\\.Percent\\.below\\.poverty\\.level\\.\\.Estimate$", ""))

Poverty_transposed2020 <- Poverty_transposed2020[, c("State", "Populationforwhompovertystatusisdetermined")]

```

```{r}
Poverty_transposed2020$State <- gsub("District.of.Columbia", "District of Columbia", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("Puerto.Rico", "Puerto Rico", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("West.Virginia", "West Virginia", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("New.Hampshire", "New Hampshire", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("North.Carolina", "North Carolina", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("South.Carolina", "South Carolina", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("Rhode.Island", "Rhode Island", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("North.Dakota", "North Dakota", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("South.Dakota", "South Dakota", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("New.Mexico", "New Mexico", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("New.Jersey", "New Jersey", Poverty_transposed2020$State)
Poverty_transposed2020$State <- gsub("New.York", "New York", Poverty_transposed2020$State)
```

## Merging all the Poverty transposed data

```{r}
# Create a list of the vehicle datasets
Poverty_data <- list(
  "2020" = Poverty_transposed2020,
  "2021" = Poverty_transposed2021,
  "2022" = Poverty_transposed2022,
  "2023" = Poverty_transposed2023
)

# Convert each year’s vehicle data into a long format and combine
Poverty_long <- bind_rows(
  lapply(names(Poverty_data), function(year) {
    Poverty_data[[year]] %>%
      mutate(Year = as.numeric(year)) # Add the year as a column
  })
)
...
Poverty_long <- Poverty_long %>%
  mutate(
    Populationforwhompovertystatusisdetermined = str_remove(Populationforwhompovertystatusisdetermined, "%"),
    Populationforwhompovertystatusisdetermined = as.numeric(Populationforwhompovertystatusisdetermined) / 100
  )

Poverty_long <- Poverty_long %>%
  rename(PercentPoverty = Populationforwhompovertystatusisdetermined)

```

```{r}
# Merge the reshaped Internet dataset with TelehealthandPopulation_Vehicle on State and Year
TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit <- merge(TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE_Transit, Poverty_long, by = c("State", "Year"), all = TRUE)
...
write_xlsx(TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit, 
           path = "TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit.xlsx")
...
### By Race
```{r}
# Define cute color palette for Race
race_colors <- c(
  "American Indian/Alaska Native" = "#A6CEE3",  # light blue
  "Asian/Pacific Islander" = "#F28D8D",  # light pink
  "Black/African American" = "#A8D08D", #green
  "Hispanic" = "#F4C318", # light yellow
  "Non-Hispanic White" = "#D1A9E2"  # light lavender
)

# Create bar plot for Telehealth Visits by Race
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE_Transit %>%
  ...
  labs(title = " ",
       x = "Race Category",
       y = "Sum of Number of Telehealth Visits (Per Capita)") +
  scale_y_continuous(labels = scales::comma) +  # Format y-axis with full numbers and commas
  scale_fill_manual(values = race_colors) +  # Apply color palette
  theme_minimal() +
  theme(legend.position = "none") +
  theme(
    axis.text.x = element_text(angle = 45, hjust = 1, size = 9),  # Increased font size
...
colors <- c(
  "Medicare & Medicaid" = "#A6CEE3",
  "Medicare Only" = "#F28D8D"   
)
...
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  ...
  labs(title = " ",
       x = "Enrollment Status",
       y = "Sum of Number of Number of Telehealth Visits (Per Capita)") +
    scale_y_continuous(labels = scales::comma) +  # Format y-axis with full numbers and commas
...
```{r}
# Define color palette for Year
year_colors <- c(
  "2020" = "#A6CEE3",  # light blue
  "2021" = "#F28D8D",  # light pink
  "2022" = "#A8D08D",   # light green
  "2023" = "#F4C318"
)

# Create bar plot for Telehealth Visits Per Capita by Year
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  ...
  labs(x = "Year",
       y = "Sum of Number of Telehealth Visits (Per Capita)") +
  scale_y_continuous(labels = scales::comma) +  # Format y-axis with full numbers and commas
  scale_fill_manual(values = year_colors) +  # Apply color palette
  theme_minimal() +
  theme(legend.position = "none")
...
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  ...
  geom_bar(stat = "identity") +
  labs(x = "Year",
       y = "Sum of Number of Eligible Services (Per Capita)") +
    scale_y_continuous(labels = scales::comma) +  # Format y-axis with full numbers and commas
  scale_fill_manual(values = year_colors) +  # Apply color palette
  theme_minimal() +
...
# Create bar plot for Rural vs Urban Telehealth Visits
...
### Multivariable including Poverty

```{r}
model1 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + UNRATE_prop, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # without State/Year fixed effects

model2 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + UNRATE_prop + State, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # with State fixed effects

model3 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + UNRATE_prop + State + Year, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # with State and Year fixed effects
...
coefz <- c("TotalOwnedPerCapita" = "Vehicles Owned",
           ...
           "HospitalCountPerCapita" = "Hospitals Count",
            "NoofPhysiciansPerCapita" = "Number of Physicians",
           "PercentPoverty" = "Percent Below Poverty Level ",
           "GDP_trillions" = "GDP",
...
vif_model <- lm(
  TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + UNRATE_prop, data = TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit
)
...
```{r}
model1 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # without State/Year fixed effects

model2 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + State, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # with State fixed effects

model3 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty + GDP_trillions + State + Year, TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit) # with State and Year fixed effects
...
coefz <- c("TotalOwnedPerCapita" = "Vehicles Owned",
           ...
           "HospitalCountPerCapita" = "Hospitals Count",
           "NoofPhysiciansPerCapita" = "Number of Physicians",
           "PercentPoverty" = "Percent Under Poverty Level")
...
# Fit the model (excluding fixed effects for accurate VIF calculation)
vif_model <- lm(
  TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + PercentPoverty, data = TelehealthandPopulation_VehicleInternetHospitalPoverty_GDPUNRATE_Transit
)
