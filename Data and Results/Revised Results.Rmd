...
```{r}
model1 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + TotalIncomePerCapita + GDP_trillions, TelehealthandPopulation_VehicleInternetHospitalIncome_GDPUNRATE_Transit) # without State/Year fixed effects

model2 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + TotalIncomePerCapita + GDP_trillions + State, TelehealthandPopulation_VehicleInternetHospitalIncome_GDPUNRATE_Transit) # with State fixed effects

model3 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + TotalIncomePerCapita + GDP_trillions + State + Year, TelehealthandPopulation_VehicleInternetHospitalIncome_GDPUNRATE_Transit) # with State and Year fixed effects
...
# Fit the model (excluding fixed effects for accurate VIF calculation)
vif_model <- lm(
  TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TransitSpendingScore + WithInternetPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + TotalIncomePerCapita + GDP_trillions, data = TelehealthandPopulation_VehicleInternetHospitalIncome_GDPUNRATE_Transit
)
