...
* Satellite Internet –  often the option of internet access in rural areas

## Create interaction term with Urban/Rural

```{r}
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE <- TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  mutate(is_rural = case_when(
    RuralUrban == "Rural" ~ 1,       # Set 1 for Rural
    RuralUrban == "Urban" ~ 0,       # Set 0 for Urban
    TRUE ~ NA_real_                  # Set NA for 'All'
  ))

# Create the interaction term for CableFiberOrDSL
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE <- TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  mutate(CableFiberOrDSL_Rural = CableFiberOrDSLPerCapita * is_rural)

# Create the interaction term for SatelliteInternetOnly
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE <- TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  mutate(SatelliteInternetOnly_Rural = SatelliteInternetOnlyPerCapita * is_rural)

# Create the interaction term for BroadBandAnyTypePerCapita
TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE <- TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE %>%
  mutate(BroadBandAnyType_Rural = BroadBandAnyTypePerCapita * is_rural)
...
```{r}
model1 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TotalRentPerCapita + BroadBandAnyType_Rural + SatelliteInternetOnly_Rural + HospitalCountPerCapita + NoofPhysiciansPerCapita + GDP_trillions + UNRATE_prop,TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # without State/Year fixed effects

model2 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TotalRentPerCapita + BroadBandAnyType_Rural + SatelliteInternetOnly_Rural + HospitalCountPerCapita + NoofPhysiciansPerCapita + GDP_trillions + UNRATE_prop + State, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State fixed effects

model3 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TotalRentPerCapita + BroadBandAnyType_Rural + SatelliteInternetOnly_Rural + HospitalCountPerCapita + NoofPhysiciansPerCapita + GDP_trillions + UNRATE_prop + State + Year, TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # with State and Year fixed effects
...
```{r}
model1 <- lm(TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TotalRentPerCapita + CableFiberOrDSLPerCapita + SatelliteInternetOnlyPerCapita + DialUpNoInternetSubsPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + GDP_trillions + UNRATE_prop,TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE) # without State/Year fixed effects
...
coefz <- c("TotalOwnedPerCapita" = "Vehicles Owned",
           "TotalRentPerCapita" = "Vehicles Rented",
           "CableFiberOrDSLPerCapita" = "Cable Fiber Or DSL", #Likely urban
           "SatelliteInternetOnlyPerCapita" = "Satellite Internet Only", # LIkely Rural
           "DialUpNoInternetSubsPerCapita" = "DialUpNoInternetSubsPerCapita", # No Internet
           "HospitalCountPerCapita" = "Hospitals Count",
           "NoofPhysiciansPerCapita" = "Number of Physicians",
           "GDP_trillions" = "GDP",
           "UNRATE_prop" = "Unemployement Rate")
...
* Rented is higher because semi high financial status but less than owning so mroe use of telehealth

* Cable fiber is switches to negative but not statistically significant anymore, Maybe because they are likely urban and more access to in person?

* Satellite switches to negative - this is an area that is lacking telehelath but could benefit because it is likely rural

* Dial Up is positive - WHY? these people don't hace internet subs...

* Hospiutal count and num of physicians GDP and UNRATE are most statistically significant

* Unrate could be positive because this is medicaid populaiton specificially
...
vif_model <- lm(
  TelehealthVisitsPerCapita ~ TotalOwnedPerCapita + TotalRentPerCapita + CableFiberOrDSLPerCapita + DialUpNoInternetSubsPerCapita + SatelliteInternetOnlyPerCapita + HospitalCountPerCapita + NoofPhysiciansPerCapita + GDP_trillions + UNRATE_prop, data = TelehealthandPopulation_VehicleInternetHospital_GDPUNRATE
)
