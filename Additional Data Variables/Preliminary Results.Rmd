...
## Cleaning Internet 2023 Dataset
...
### Cleaning Internet 2022 Dataset
...
### Cleaning Internet 2021 Dataset
...
### Cleaning Internet 2020 Dataset
...
```{r}
colnames(TelehealthandPopulation_VehicleInternet)
```
...
TelehealthandPopulation_VehicleInternet %>%
  ...
  geom_bar(stat = "identity") +
  labs(title = "Telehealth Visits by State (per 100k people)",
       x = "State",
       y = "Number of Telehealth Visits") +
    scale_y_continuous(labels = scales::comma) +  # Format y-axis with full numbers and commas
  theme_minimal() +
...
TelehealthandPopulation_VehicleInternetHospital %>%
  ...
  geom_bar(stat = "identity") +
  labs(title = "Number of Hospitals by State (per 100k people)",
       x = "State",
...
model1<-lm(TelehealthVisits~Race, TelehealthandPopulation_VehicleInternet) # without Income fixed effects
model2 <- lm(TelehealthVisits~Race + State, TelehealthandPopulation_VehicleInternet) # with State fixed effects
model3<-lm(TelehealthVisits~Race + State + Year, TelehealthandPopulation_VehicleInternet) # with State and Year fixed effects
...
table_output <- modelsummary(regz,
             title = "Effect of State and Year on Races' Impact on Number of Telehealth Visits",
             estimate = "{estimate}{stars}",
             gof_map = gofz,
             add_rows = rowz,
             coef_omit = "^(Race|State)$")
